...
これらの引数を設定して`adf_test_flow()`自作関数を実行すると、ADF検定フローの結果を記載したtibble形式のデータフレームが返ります。trendモデル3種類、driftモデル3種類、noneモデル1種類の合計7種類のADF検定について、`y`に指定した時系列データが$I(0)$過程、$I(1)$過程、$I(2)$過程のどれに従うかが判断できるまで階差次数を増やして検定を繰り返し、各検定の結果に基づく判断経路を「＊」で示します。最後に「＊」が記載されている箇所が最終的な判断になります。
...
### 実証例 村尾 6.7 ADF検定 {.unnumbered}

定義した自作関数`adf_test_flow()`を用い、村尾（2019）6.7「拡張ディッキー＝フラー検定の例」（P.141）及び、6.10「Rによる拡張ディッキー＝フラー検定」（P.147）の例を再現します。

村尾（2019）6.7及び6.10では、`vars`パッケージの`Canada`データセットに含まれるカナダの実質労働生産性（`prod`）データに対してADF検定フローを適用しています。なお、ラグ次数選択基準の`lag_criterion`には赤池情報量規準を意味する`"AIC"`を指定します。
...
自己相関プロットや偏自己相関プロットは系列相関の有無を視覚的に判断できる便利な方法ですが、プロット上に表示される95％信頼区間は逐次検定用であり、自己相関の数が大きくなると多重検定の問題が生じます（t検定を複数回行うと設定された有意水準5％よりも高い頻度で第1種の過誤が発生する）。つまり、実際には系列相関がないにも関わらず、どこかのラグ次数で帰無仮説（自己相関係数が0）を棄却してしまう確率が増加します（西山 他（2019））。
...
を一度に検定する必要があります。対立仮説は「少なくとも一つの自己相関係数が0ではない」です。この結合帰無仮説の検定が、**リュン＝ボックス検定（Ljung-Box test）**です。なお、過去には「ダービン・ワトソン比」で系列相関の有無を判断する方法が用いられていましたが、現在ではLjung-Box検定を用いるのが一般的です（西山 他（2019）P.484）。
...
Rでは、Engle-Granger検定を一般化した**Phillips-Ouliaris検定（PO検定）**が用いられます（馬場（2018）P.142）。PO検定の帰無仮説は「共和分なし」（回帰残差が単位根をもつ）、対立仮説は「共和分あり」（回帰残差が単位根をもたない）です。

### 実証例 西山 12.3 見せかけの回帰 {.unnumbered}
...
### Johansen検定 {.unnumbered}

変数間の共和分関係を判断するためのもう一つの方法が、**ヨハンセン検定（Johansen test）**です。Engle-Granger検定が2変数の間の共和分の有無を判断する方法であるのに対し、Johansen検定は2つ以上の多変数の間に共和分関係がいくつ存在するか判断します。

Johansen検定には**トレース検定（trace test）**と**最大固有値検定（maximum eigenvalue test）**の2つの検定方法があります。
...
Johansen検定の具体的なフローは次のとおりです。

まず、分析対象のデータセットに含まれる各変数について単位根検定を行い、定常な$I(0)$変数と非定常な$I(1)$変数がそれぞれいくつあるか確認します。データセットに$I(0)$変数が$K_0$個、$I(1)$変数が$K_1$個含まれるとすると、Johansen検定で検出される共和分関係の数の最小値は$K_0$、最大値は$K_0+K_1-1$になります。

次に、Johansen検定に必要な事前情報としてラグ次数$p$を確認します。ラグ次数$p$は、次に説明するベクトル自己回帰（VAR）モデルを暫定的に推定して決定します。ラグ次数を選択するための情報量規準には、赤池情報量規準（Akaike Information Criterion、AIC）、赤池最終予測誤差（Akaike's Final Prediction Error、FPE）、ハナン＝クイン情報量規準（Hannan-Quinn Information Criterion、HQ）、シュワルツのベイズ情報量規準（Schwarz Bayesian Information Criterion、SC）を用います。これら3つの情報量規準が選ぶラグ次数には、\$SC \leq HQ \leq AIC \fallingdotseq FPE \$の関係があります。時系列データのサンプル数が大きい場合はSCを、小さい場合はAICかFPEを、どちらか迷う場合はHQを選択してください（村尾（2019）P.117）。

また、同様に事前情報として、共和分関係を表す式に定数項・トレンド項を含むか否かを、想定する共和分関係に基づいて設定します。定数項・トレンド項の有無で検定の棄却点が変化し、結果が変わりうるため、設定には注意を要します。

最後にJohansen検定を実行します。帰無仮説における共和分関係の数を$H_0:r=0$、$H_0:r=1$、$H_0:r=2$と一つずつ増やしてトレース検定・最大固有値検定を繰り返し、帰無仮説$H_0$が初めて棄却できなくなった時の$r$を共和分関係の数と判断します。トレース検定で大まかな共和分関係の数を把握し、最大固有値検定で細かく見て最終的な共和分関係の数を決定します。

なお、Johansen検定の詳細については、村尾（2019）7.3「ヨハンセン検定」（P.153〜）、西山 他（2019）第12章「ヨハンセン検定」（P.608〜）及び「補論」（P.626〜）を参照してください。

### Johansen検定フロー自作関数 {.unnumbered}

本書では、Johansen検定フローを分析対象の時系列データセットに自動で適用する自作関数`johansen_test_flow()`を定義します。

`johansen_test_flow()`自作関数の引数は次のとおりです。

-   `data`：Johansen検定を適用する時系列データセット。数値型データを格納したdata.frame形式（tibble形式を含む）もしくはts形式のデータセットを指定します。
...
-   `johansen_spec`：Johansen検定において、共和分関係の均衡からの誤差修正が一時的（`"transitory"`）か、長期的（`"longrun"`）かを指定します。
-   `sig_level`：有意水準。0.01、0.05、0.1の3つのうち1つを指定します。

これらの引数を設定して`johansen_test_flow()`自作関数を実行すると、共和分関係式にトレンド項を含むモデル、定数項を含むモデル、トレンド項・定数項どちらも含まないモデル、の3種類のJohansen検定フローの結果を格納したリストが返ります。

```{r}
library(docstring)

# 時系列データセットに対しJohansen検定フローを自動で適用する関数
...
### 実証例 村尾 7.4 Johansen検定 {.unnumbered}

定義した自作関数`johansen_test_flow()`を用い、村尾（2019）7.4「ヨハンセン検定の例」（P.158）及び、7.7「Rによるヨハンセン検定」（P.166）の例を再現します。

村尾（2019）7.4及び7.7では、`vars`パッケージの`Canada`データセットに含まれるカナダの実質労働生産性（`prod`、自然対数×100）、雇用者数（`e`、自然対数×100）、失業率（`U`、％）、実質賃金（`rw`、自然対数×100）の4変数（サンプル数84）に対し、Johansen検定を適用しています。

検定対象である4変数は、全て事前にADF検定で$I(1)$過程であることが確認されています。したがって、共和分関係の数は最大で3になります。`johansen_test_flow()`自作関数における事前推定VARモデルのオプションでは、ラグ次数選択基準の`lag_criterion`にHannan-Quinn情報量規準を意味する`"HQ"`を、確定項の`ver_type`にトレンド項と定数項の両方を意味する`"both"`を指定します。また、Johansen検定のオプションでは、誤差修正スペックを意味する`johansen_spec`に「一時的」を意味する`"transitory"`を指定します。なお、村尾（2019）7.4及び7.7では検定の優位水準を10％としていますが、ここでは5％を指定します。
...
# CanadaにJohansen検定フローを適用
jo_canada <- johansen_test_flow(data = Canada, # Johansen検定を行う時系列データセット
                                lag_criterion = "HQ", # 事前推定VARモデルでラグ次数を決定する情報量規準
                                lags_max = 10, # 最大ラグ次数
                                var_type = "both", # 事前推定VARモデルに含む確定項
                                johansen_spec = "transitory", # Johansen検定のスペック
                                sig_level = 0.05 # 有意水準
                                )
...
誤差項$u_{1t}$と$u_{2t}$は定常なホワイトノイズであり、過去の自身の誤差項とは相関していません。ただし、同時点の誤差項同士の相関は許容します。
...
### モデル推定後の分析 {.unnumbered}

#### Granger因果性 {.unnumbered}

**グレンジャー因果性（Granger causality）**とは、予測精度の改善という観点から見た、時系列データに特有の因果性の概念です（西山 他（2019）P.584）。

Granger因果性では、「原因は結果に先立って起こる」、「原因には結果に関するユニークな情報が含まれている」という2つのアイデアをもとにして、理論を必要とせずに時系列データのみから因果性の有無を判断します（村尾（2019）P.369）。

具体例として、ある事象の原因と考えられる変数$X_t$と、結果と考えられる変数$Y_t$に関する次のモデルを考えます。

$$
\begin{aligned}
Y_t &= \phi_1Y_{t-1} \quad \quad \quad \quad \; + u_1 \\
Y_t &= \phi_1Y_{t-1} + \phi_2X_{t-1} + u_2
\end{aligned}
$$

ここで、$u_1$は過去の$Y_t$だけを使った$Y_t$の予測残差、$u_2$は過去の$X_t$も使った$Y_t$の予測残差です。この2つの予測誤差の残差平方和（Mean Squared Error、MSE）を比較し、$X_t$を予測に使うことで予測残差が有意に小さくなったかどうかを検定するのが、Granger因果性検定です。

Granger因果性検定は通常、原因の変数（上の例の収入$X_t$）と結果の変数（上の例の消費$Y_t$）を含むVARモデルを構築し、「結果の変数の回帰式における原因の変数の係数がゼロ」（Granger因果性がない）との帰無仮説を検定する方法で行います。

なお、Granger因果性を用いて分析する際は、次の点に注意してください。

1.  Granger因果性検定が使用できるのは、定常な時系列データのみである（馬場（2018）P.147）。

2.  原因と結果の変数両方に影響する第3の変数がある場合は、原因・結果変数のGranger因果性が有意であったとしても、真の因果関係はない。例えば、原因：津波警報と結果：津波（真の原因は地震）、原因：雷光と結果：雷鳴（真の原因は落雷現象）など（村尾（2019）P.372）。

3.  Granger因果性検定の結果は、VARモデルに含まれる他の変数によって変化する。結果の変数と相関の強い第3の変数をモデルに追加すると、原因の変数のGranger因果性は低下する（村尾（2019）P.372）。

4.  Granger因果性検定の結果をそのまま政策判断に用いるべきでない。政策手段である原因の変数が内生変数の場合（例えば貨幣供給量や名目金利など）、原因の変数の回帰式の誤差項には政策変化以外の要因が含まれており、純粋な政策ショックの効果を捉えることができない。政策ショックの影響を分析するには、次のインパルス応答を用いる必要がある（西山 他（2019）P.590）。

#### インパルス応答 {.unnumbered}

**インパルス応答（impulse response）**は、VARモデルのある変数に急な変動が発生した場合に、他の変数へ時間を通じてどのような影響が生じるかを定量的に評価する手法です。

VARモデルでは各回帰式に誤差項がありますが、通常は各回忌式の誤差項同士に相関があります。これは、各誤差項の中に複数の変数のショックが含まれていることを意味しています。インパルス応答を計算するには、VARモデルに外部から追加的な制約を加え、誤差項から単一の変数のショック（独立ショック）を抽出する必要があります。この手続きをショックの**識別（identification）**と言います（西山 他（2019）P.611）。

インパルス応答の計算では「短期制約」と呼ばれる制約が用いられます。例として、変数$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$の3変数で構成される次の$VAR(1)$モデルを考えます。

$$
\begin{aligned}
Y_{1,t} &= \beta_{10} + \beta_{11}Y_{1,t-1} + \beta_{12}Y_{2,t-1} + \beta_{13}Y_{3,t-1} + u_{1,t} \\
Y_{2,t} &= \beta_{20} + \beta_{21}Y_{1,t-1} + \beta_{22}Y_{2,t-1} + \beta_{23}Y_{3,t-1} + u_{2,t} \\
Y_{3,t} &= \beta_{30} + \beta_{31}Y_{1,t-1} + \beta_{32}Y_{2,t-1} + \beta_{33}Y_{3,t-1} + u_{3,t}
\end{aligned}
$$

各回帰式の誤差項$u_{1,t}$、$u_{2,t}$、$u_{3,t}$は互いに相関しており、変数$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$の独立ショックではありません。ここで、各回帰式の誤差項$u_{1,t}$、$u_{2,t}$、$u_{3,t}$が、変数$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$の独立ショック$\epsilon_{1,t}$、$\epsilon_{2,t}$、$\epsilon_{3,t}$によって、次のように表されるとします。

$$
\begin{aligned}
u_{1,t} &= c_{11}\epsilon_{1,t} + c_{12}\epsilon_{2,t} + c_{13}\epsilon_{3,t} \\
u_{2,t} &= c_{21}\epsilon_{1,t} + c_{22}\epsilon_{2,t} + c_{23}\epsilon_{3,t} \\
u_{3,t} &= c_{31}\epsilon_{1,t} + c_{32}\epsilon_{2,t} + c_{33}\epsilon_{3,t}
\end{aligned}
$$

このとき、推定した誤差項$u_{1,t}$、$u_{2,t}$、$u_{3,t}$から独立したショック$\epsilon_{1,t}$、$\epsilon_{2,t}$、$\epsilon_{3,t}$を識別するには、$c_{11}$〜$c_{33}$の9個のパラメータが必要です。一方、いまデータから推定が可能なパラメータは、次に示す誤差項の分散・共分散、

$$
\begin{aligned}
& \sigma^2_1 = c^2_{11} + c^2_{12} + c^2_{13} \\
& \sigma^2_2 = c^2_{21} + c^2_{22} + c^2_{23} \\
& \sigma^2_3 = c^2_{31} + c^2_{32} + c^2_{33} \\
& \sigma_{12} = c_{11}c_{21} + c_{12}c_{22} + c_{13}c_{23} \\
& \sigma_{13} = c_{11}c_{31} + c_{12}c_{32} + c_{13}c_{33} \\
& \sigma_{23} = c_{21}c_{31} + c_{22}c_{32} + c_{23}c_{33}
\end{aligned}
$$

の合計6個です。したがって、独立ショックを識別するには、追加で3個の制約を加える必要があります。そこで、$c_{12}$、$c_{13}$、$c_{23}$をゼロとする制約を加えると、誤差項と独立ショックの関係は、

$$
\begin{aligned}
u_{1,t} &= c_{11}\epsilon_{1,t} \\
u_{2,t} &= c_{21}\epsilon_{1,t} + c_{22}\epsilon_{2,t} \\
u_{3,t} &= c_{31}\epsilon_{1,t} + c_{32}\epsilon_{2,t} + c_{33}\epsilon_{3,t}
\end{aligned}
$$

となります。すると、残りの$c_{11}$、$c_{21}$、$c_{22}$、$c_{31}$、$c_{32}$、$c_{33}$は誤差項の分散・共分散を用いて解くことができ、独立ショックを識別することが可能になります。

このようにして識別した独立ショックを用い、$t-d$期において変数$Y_{1,t-d}$、$Y_{2,t-d}$、$Y_{3,t-d}$発生した独立ショック$\epsilon_{1,t-d}$、$\epsilon_{2,t-d}$、$\epsilon_{3,t-d}$が$t$期の変数$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$に与える影響を計算したものが、インパルス応答関数です。

ここで、$t-d$期に変数$Y_{j,t-d}$に発生した独立ショック$\epsilon_{j,t-d}$が$t$期の変数$Y_{i,t}$に及ぼす影響を、$Y_{i,t}$の$Y_{j,t-d}$に対するインパルス応答と呼び、関数$C_{ij}(d)$で表します。

例えば$d = 0$の時、$t-0 = t$期に発生した独立ショックが$Y_{3,t}$に与える影響$C_{31}(0)$、$C_{32}(0)$、$C_{33}(0)$は、$VAR(1)$モデルの第3式の誤差項$u_{3,t}$に$c_{31}\epsilon_{1,t} + c_{32}\epsilon_{2,t} + c_{33}\epsilon_{3,t}$を代入した、

$$
Y_{3,t} = \beta_{30} + \beta_{31}Y_{1,t-1} + \beta_{32}Y_{2,t-1} + \beta_{33}Y_{3,t-1} + c_{31}\epsilon_{1,t} + c_{32}\epsilon_{2,t} + c_{33}\epsilon_{3,t}
$$

における$\epsilon_{1,t}$、$\epsilon_{2,t}$、$\epsilon_{3,t}$の係数、すなわち、
...
になります。一方、同じ$t-0 = t$期に発生した独立ショックが$Y_{1,t}$に与える影響は、$c_{12} = 0$、$c_{13} = 0$の制約により、
...
C_{13}(0) &= 0
\end{aligned}
$$ になります。同様に$Y_{2,t}$に与える影響は、$c_{23} = 0$の制約により、
...
になります。これらが意味しているのは、$Y_{3,t}$は同時点の$Y_{1,t}$、$Y_{2,t}$から影響を受けるが、$Y_{2,t}$は$Y_{1,t}$からしか影響を受けず、$Y_{1,t}$は$Y_{2,t}$、$Y_{3,t}$どちらからも影響を受けないということです。すなわち、$c_{12}$、$c_{13}$、$c_{23}$をゼロとする制約は、3つの変数が$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$の順に外性性が高いとの前提を置いていることになります。

このように、短期制約を課して識別した独立ショックを用いて求めるインパルス応答関数を、**直交化インパルス応答関数（orthogonal impulse response function）**と呼びます。通常、インパルス応答関数と言えばこの直交化インパルス応答関数を指します。[直交化インパルス応答関数では、上に示したように変数を外性性が高いと考えられる順番に並べることが重要であり、変数の順序によってインパルス応答の結果が変化します。]{.underline}

なお、変数の順序に依存しないインパルス応答の計算方法として、**一般化インパルス応答関数（generalized impulse response function）**という手法もあります。詳細は村尾（2019）第11章を参照してください。

#### 分散分解 {.unnumbered}

**分散分解（variance decomposition）**は、ある変数の変動に対して他の変数がどの程度影響しているかを定量的に分析する手法です（村尾（2019）P.339）。
...
例えば3変数$Y_{1,t}$、$Y_{2,t}$、$Y_{3,t}$のVARモデルにおいて、$t-d$期に変数$Y_{j,t-d}$に発生した独立ショック$\epsilon_{j,t-d}$が$t$期の変数$Y_{i,t}$に及ぼす影響を、$Y_{i,t}$の$Y_{j,t-d}$に対するインパルス応答と呼び、関数$C_{ij}(d)$で表すとき、$h$期先の変数$Y_{1,t}$のMSFEに占める$\epsilon_{2,t}$の分散寄与度（RVC）は、

$$
\frac{\Sigma^{h-1}_{d=0}{C_{12}(d)^2}}{\Sigma^{h-1}_{d=0}{C_{11}(d)^2} + \Sigma^{h-1}_{d=0}{C_{12}(d)^2} + \Sigma^{h-1}_{d=0}{C_{13}(d)^2}}
$$

で計算できます。

### VARモデルの分析フロー {.unnumbered}
...
2.  ラグ次数選択
    ...
    最初に自己回帰（AR）過程のラグ次数$p$を選択します。選択したラグ次数$p$は、この後に行う単位根検定、共和分検定、モデル推定で使用します。

    ラグ次数を選択するための情報量規準には、赤池情報量規準（Akaike Information Criterion、AIC）、赤池最終予測誤差（Akaike's Final Prediction Error、FPE）、ハナン＝クイン情報量規準（Hannan-Quinn Information Criterion、HQ）、シュワルツのベイズ情報量規準（Schwarz Bayesian Information Criterion、SC）を用います。これら3つの情報量規準が選ぶラグ次数には、\$SC \leq HQ \leq AIC \fallingdotseq FPE \$の関係があります。時系列データのサンプル数が大きい場合はSCを、小さい場合はAICかFPEを、どちらか迷う場合はHQを選択してください（村尾（2019）P.117）。
...
3.  単位根検定
    ...
    -   全ての変数が$I(0)$のケース：　レベルVARモデルを適用（フロー5へ）
    -   1つの変数が$I(1)$：　階差VARモデルを適用（フロー5へ）
    -   2つ以上の変数が$I(1)$：　共和分検定を行い、$I(1)$変数間の共和分関係の有無を確認（フロー4へ）

4.  共和分検定

    単位根検定で2つ以上の$I(1)$変数があると判断した場合は、共和分検定を行い、$I(1)$変数間に共和分関係があるか判断します。
    
    Rでは本書で自作したJohansen検定フロー関数の`johansen_test_flow()`関数を用いて共和分検定を行います。
    
    共和分検定の結果に基づき、適用するモデルが変わります。
    
    * 共和分関係がないケース：　階差VARモデルを適用（フロー5へ）
    * 共和分関係があるケース：　VECモデルを適用（フロー5へ）
...
5.  モデル推定
    ...
    ここまでに実施した一連の**診断的検定（diagnostic test）**に基づき、レベルVARモデル、階差VARモデル、VECモデルの推定を行います。

    レベルVARモデルと階差VARモデルは、`vars`パッケージの`VAR()`関数で推定します。階差VARモデルの場合は事前に変数を階差変換しておきます。
    
    VECモデルは、Johansen検定に用いる`urca`パッケージの`ca.jo()`関数で共和分ランクを決定したうえで、`ca.jo()`関数で出力したオブジェクトを`urca::cajorls()`関数の入力として用い、VECモデルを推定します。

6.  誤差項の検定

    VARモデルが推定できたら、誤差項の推定量である残差系列を用いて、誤差項に系列相関がないか、分散が均一か、正規分布に従っているかを検定します。これらの誤差項の過程が満たされない場合は、変数やラグ次数を選択し直し、再度モデル推定を行う必要があります。
    
    検定に用いるRの関数は次のとおりです。
    
    * 系列無相関 Breusch-Godfrey LM検定（帰無仮説：系列相関なし）：　`vars`パッケージの`serial.test()`関数
    * 均一分散 ARCH-LM検定（帰無仮説：均一分散）：　`vars`パッケージの`arch.test()`関数
    * 正規分布 Jarque-Bera検定（帰無仮説：正規分布）：　`vars`パッケージの`normality.test()`関数
    
    VARモデルの場合は、`vars::VAR()`関数で出力したオブジェクトを上記の各関数に入力します。VECモデルの場合は、`urca::ca.jo()`関数で出力したアウトプットを`vars::vec2var()`関数でVARモデルのオブジェクトに変換し、上記の各関数に入力します。

7.  モデル推定後の分析

    推定したモデルを用いて、インパルス応答、予測誤差の分散分解、Granger因果性検定などの追加的な分析を行います。（直交化）インパルス応答には`vars::irf()`関数、予測誤差の分散分解には`vars::fevd()`関数、Granger因果性検定には`vars::causality()`関数を用います。
    
    なお、Granger因果性検定にはVARモデルを推定する`vars::VAR()`関数の出力オブジェクトのみ用いることができます。VECモデルを`vars::vec2var()`関数で変換したオブジェクトではGranger因果性検定は実施できません。

7.  予測精度評価

    推定したモデルを用いて予測値を作成し、分割しておいたテストデータの実績値と比較して予測精度を評価します。評価指標にはRMSE（Root Mean Square Error）などが用いられます。加えて、過去の平均値や前期の値を予測値として用いるナイーブ予測（VARなどの複雑なモデルを使わない予測）の精度を上回ることを確認します。予測値の作成には、`vars`パッケージの`predict()`関数を用います。
