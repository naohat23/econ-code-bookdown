...
Rで外部との通信が可能であることをチェックするには、`httr`パッケージを使用して次のコードを実行します。ステータス200が表示されれば、外部通信が成功していることを意味します。

```{r, paged.print=FALSE}
library(httr)
status_code(GET("https://www.google.com"))
```
...
-   その他
    ...
    -   [httr](https://httr.r-lib.org/)：HTTPリクエスト
