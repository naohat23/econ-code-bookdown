...
packages <- c(
  # tidyverse系
  "lubridate", "magrittr", "tidyverse", "tsibble", "readxl",
  # 図表系
  "esquisse", "geofacet", "ggplotgui", "ggpubr", "ggsci", "ggrepel", "hexbin", "lemon", "RColorBrewer",
  # 統計系
  "corrplot", "corrr", "DataExplorer", "estimatr", "fpp2", "GGally", "lmtest", "mFilter", "mgcv", "plm", "psych", "sandwich", "seasonal", "sigmoid", "SmartEDA", "strucchange", "tidyquant", "vars", "urca",
  # データパッケージ
  "nycflights13",
  # その他
  "openxlsx", "pbapply"
...
`ggplot2`パッケージに含まれる`economics`データセットは、1967年7月から2015年4月までの米国の消費・雇用関連の時系列データ（月次）です。日付（date）、[個人消費支出（pce）](https://fred.stlouisfed.org/series/PCE)、[人口（pop）](https://fred.stlouisfed.org/series/POP)、[個人貯蓄率（psavert）](https://fred.stlouisfed.org/series/PSAVERT/)、[失業期間の中央値（uempmed）](https://fred.stlouisfed.org/series/UEMPMED)、[失業者数（unemploy）](https://fred.stlouisfed.org/series/UNEMPLOY)のデータが格納されています。データ構造はtibble形式です。
...
`seasonal`パッケージに含まれる`unemp`データセットは、1990年1月から2016年11月までの米国の失業者数（単位：千人）の時系列データ（月次、原数値）です。データ構造はts形式です。
...
`fpp2`パッケージに含まれる`gasoline`データセットは、1991年2月2日から2017年１月20日までの米国ガソリン生産量（単位：百万バレル／日）の時系列データ（週次）です。データ構造はts形式です。

#### Canada {-}

`vars`パッケージに含まれる`Canada`データセットは、1980年1〜3月期から2000年10〜12月期までのカナダのマクロ経済時系列データ（四半期）です。実質労働生産性（prod、自然対数×100）、雇用者数（e、自然対数×100）、失業率（U、％）、実質賃金（rw、自然対数×100）のデータが格納されています。データ構造はts形式です。
