...
しかし、`ggplot2`パッケージの`geom_boxplot()`で作成すると、ひげの下端・上端は箱の下端・上端から1.5×IQRの範囲内にある最小・最大のデータの位置を表すことに注意が必要です。その範囲外にあるデータは外れ値として扱われます。なお、IQRは四分位範囲（第3四分位数-第1四分位数）を意味します。
...
箱ひげ図にジッターグラフで個別データを追加する場合は、データの重複表示を避けるため、`geom_boxplot()`関数の`outlier.shape`引数に`NA`を指定して、箱ひげ図による外れ値のマーカーを非表示にします。
...
横並び箱ひげ図にジッターグラフで個別データを追加する場合は、`geom_jitter()`関数の`height`引数と`width`引数を削除し、`position`引数に`position_jitterdodge()`関数を指定します。
...
ggplot(data = mpg,
  ...
  geom_violin(scale = "area", # ヴァイオリンの大きさ（area：すべての面積が同じ、count：データ数に比例、width：すべての幅が同じ）
...
ggplot(data = mpg,
  ...
  geom_point() +
  # ファセット
  facet_rep_grid(rows = vars(class), # 行方向にグラフを分ける変数名を指定
...
ggplot(data = mpg,
  ...
  geom_point() +
  # ファセット
  facet_rep_grid(cols = vars(class), # 列方向にグラフを分ける変数名を指定
                 scales = "fixed", # 目盛設定（fixed：全グラフ共通、free：全グラフ独立、free_x：X軸のみ独立、free_y：Y軸のみ独立）
                 repeat.tick.labels = FALSE # 目盛表示（TRUE：すべてのグラフに表示、FALSE：端のグラフのみ表示
                 )
```
...
ggplot(data = mpg,
  ...
  geom_point() +
  # ファセット
  facet_rep_grid(rows = vars(year), # 行方向にグラフを分ける変数名を指定
                 cols = vars(class), # 列方向にグラフを分ける変数名を指定
                 scales = "fixed", # 目盛設定（fixed：全グラフ共通、free：全グラフ独立、free_x：X軸のみ独立、free_y：Y軸のみ独立）
...
ggplot(data = mpg,
  ...
  geom_point() +
  # ファセット
  facet_rep_wrap(facets = ~ manufacturer, # グラフを分ける変数名を指定
                 nrow = 3, # 行方向のグラフ数
                 ncol = 5, # 列方向のグラフ数
                 scales = "fixed", # 目盛設定（fixed：全グラフ共通、free：全グラフ独立、free_x：X軸のみ独立、free_y：Y軸のみ独立）
                 repeat.tick.labels = TRUE # 目盛表示（TRUE：すべてのグラフに表示、FALSE：端のグラフのみ表示
