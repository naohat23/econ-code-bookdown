...
変数の値や属性に応じて色を自動で変化させるには、`geom`関数ファミリーでグラフを作成したあとに、`+`演算子で`scale_color`関数ファミリー（線の色）や`scale_fill`関数ファミリー（塗りつぶしの色）をつなぎます。色の変化に対応する変数が離散型か連続型かによって、どの`scale_color/fill`関数を用いるかが変わります。
