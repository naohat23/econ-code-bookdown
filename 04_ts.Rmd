...
`seasonal`パッケージのサンプルデータセット`unemp`（米国の失業者数、原数値）のトレンド推定を行います。

### HPフィルター {-}

HPフィルター（Hodrick Prescott Filter）は、時系列データのトレンド成分と循環成分を推定する手法です。
...
HPフィルターを適用するには、時系列データに対し、`mFilter`パッケージの`hpfilter()`関数を使用します。データ型は数値型ベクトル、ts型どちらでもOKです。

推定したトレンド成分、循環成分は、推定結果を格納したオブジェクトに、それぞれ`trend`、`cycle`の名称で格納されています。

```{r}
result <- mFilter::hpfilter(x = seasonal::unemp, # HPフィルターを適用する時系列データ（数値型ベクトル、ts型）
                            type = "lambda",
